...
def planner_node(state: ResearchState) -> dict:
    ...
    print("\n→ Planner: Planning parallel research...")
    notify("planner", "done")
    return {}
...
def aggregator_node(state: ResearchState) -> dict:
    print("\n→ Aggregator: Combining all research (Fan-in)...")
    aggregated = f"""
...
def build_parallel_graph():
    ...
    Planner → [Trends + News + Stats + Examples] → Aggregator → Writer
    All 4 research agents run simultaneously (Fan-out)
    Aggregator combines all results (Fan-in)
    """
    ...
    # Fan-out — planner triggers all 4 simultaneously
